...
const Invoice = ({ order, branchName, billerName }) => {
  ...
  const totalPayable = Math.round(totalBeforeRound);
  const roundOff = totalPayable - totalBeforeRound;

  const groupedItems = () => {
    const groups = {};
    order.items.forEach(item => {
      const key = `${item.menuItem?._id}-${item.price}`;
      if (!groups[key]) {
      ...
      <div className="border-t border-gray-900 mt-6 pt-6 space-y-3">
        ...
        <div className="mt-6">
          <p className="text-base font-normal">
            Paid via {order.paymentMethod === 'upi' ? 'Other [UPI]' : 
